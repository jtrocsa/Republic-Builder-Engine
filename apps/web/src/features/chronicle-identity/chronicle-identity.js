...
const mentorSprite = new URL("../../assets/chronicle-sprites/field-mentor.png", import.meta.url)
  .href;
const codexSprite = new URL("../../assets/chronicle-sprites/chronicle-codex.png", import.meta.url)
  .href;
...
const FIELD_ASSETS = {
  a: {
    down: {
      idle: new URL(
        "../../assets/chronicle-sprites/field/chronicler-a-down-idle.png",
        import.meta.url
      ).href,
      step: new URL(
        "../../assets/chronicle-sprites/field/chronicler-a-down-step.png",
        import.meta.url
      ).href,
    },
    up: {
      idle: new URL(
        "../../assets/chronicle-sprites/field/chronicler-a-up-idle.png",
        import.meta.url
      ).href,
      step: new URL(
        "../../assets/chronicle-sprites/field/chronicler-a-up-step.png",
        import.meta.url
      ).href,
    },
    side: {
      idle: new URL(
        "../../assets/chronicle-sprites/field/chronicler-a-side-idle.png",
        import.meta.url
      ).href,
      step: new URL(
        "../../assets/chronicle-sprites/field/chronicler-a-side-step.png",
        import.meta.url
      ).href,
    },
  },
  b: {
    down: {
      idle: new URL(
        "../../assets/chronicle-sprites/field/chronicler-b-down-idle.png",
        import.meta.url
      ).href,
      step: new URL(
        "../../assets/chronicle-sprites/field/chronicler-b-down-step.png",
        import.meta.url
      ).href,
    },
    up: {
      idle: new URL(
        "../../assets/chronicle-sprites/field/chronicler-b-up-idle.png",
        import.meta.url
      ).href,
      step: new URL(
        "../../assets/chronicle-sprites/field/chronicler-b-up-step.png",
        import.meta.url
      ).href,
    },
    side: {
      idle: new URL(
        "../../assets/chronicle-sprites/field/chronicler-b-side-idle.png",
        import.meta.url
      ).href,
      step: new URL(
        "../../assets/chronicle-sprites/field/chronicler-b-side-step.png",
        import.meta.url
      ).href,
    },
  },
...
const FIELD_PROP_ASSETS = {
  ...
  mentor: new URL("../../assets/chronicle-sprites/field/field-mentor-idle.png", import.meta.url)
    .href,
};
...
const FIELD_PROPS = [
  ...
  { type: "tent", x: 9, y: 5, label: "Chronicle field tent", block: true, scale: 1.3 },
  { type: "crate", x: 10, y: 7, label: "Field supplies", block: true, scale: 0.84 },
  { type: "lantern", x: 7, y: 6, label: "Field lantern", scale: 0.76 },
...
function wordCountHint(value, limit) {
  return value.length > limit
    ? `Longer than the recommended ${limit} characters. Preview it on a Chromebook.`
    : "";
}
      ...
      <form class="ci-id-author__form" id="ciIdentityAuthorForm">
        ${contentFields(content)
          .map(({ path, label, value }) => {
            const warning = wordCountHint(
              value,
              path.includes("Dialogue") ? 280 : path.includes("subtitle") ? 130 : 120
            );
            return `<label>
...
function positionStyle({ x, y }) {
  return `left:${(((x + 0.5) / FIELD_COLUMNS) * 100).toFixed(4)}%;top:${(((y + 0.5) / FIELD_ROWS) * 100).toFixed(4)}%;`;
}
...
function fieldTileMarkup() {
  return FIELD_MAP.flatMap((row, y) =>
...
function fieldPropsMarkup() {
  return FIELD_PROPS.map(
    (prop) =>
      `<img class="ci-field-prop ci-field-prop--${prop.type}" style="${positionStyle(prop)}--prop-scale:${prop.scale ?? 1};" src="${FIELD_PROP_ASSETS[prop.type]}" alt="" aria-hidden="true" />`
  ).join("");
}
...
function createFieldScreen(content, profile, state) {
  ...
  const objective = state.completeNotice
    ? "Field entry complete. Case briefing will be added in the next milestone."
    : content.field.objective;
...
export function mountChronicleIdentity(app, { onReturn } = {}) {
  ...
  const setAuthorStatus = (text) => {
    const status = app.querySelector("#ciIdentityAuthorStatus");
    if (status) status.textContent = text;
  };
  const saveContent = (text = "Saved locally in this browser.") => {
    window.clearTimeout(autosaveTimer);
    writeLocalContent(CONTENT_KEY, content);
    setAuthorStatus(text);
  };
  const scheduleContentSave = () => {
    window.clearTimeout(autosaveTimer);
    setAuthorStatus("Saving draft locally…");
    autosaveTimer = window.setTimeout(
      () => saveContent("Saved locally. Export JSON when you want a portable backup."),
      AUTOSAVE_DELAY
    );
  };
  ...
  const updateFieldPosition = () => {
    ...
    sprite.src = fieldSpriteFor(
      profile.appearance,
      fieldState.facing,
      fieldState.moving ? fieldState.step : false
    );
    ...
    if (contact && !fieldState.spoke)
      contact.textContent = isNear
        ? "You are close enough to open a secure field channel."
        : "Move closer to the field mentor to establish contact.";
    if (talk) {
  ...
  const render = () => {
    const stage =
      screen === "identity"
        ? createIdentityScreen(content, profile, message)
        : screen === "registration"
          ? createRegistrationScreen(content, profile)
          : createFieldScreen(content, profile, fieldState);
    app.innerHTML = renderShell(content, authorOn, stage);

    app.querySelectorAll("[data-appearance]").forEach((button) =>
      button.addEventListener("click", () => {
        profile = writePlayerProfile({ ...profile, appearance: button.dataset.appearance });
        message = "";
        render();
      })
    );
    app.querySelector("#ciPlayerName")?.addEventListener("input", (event) => {
      profile = writePlayerProfile({ ...profile, name: event.currentTarget.value });
    });
    app.querySelector('[data-identity-action="back"]')?.addEventListener("click", () => {
      activeKeyCleanup?.();
      onReturn?.();
    });
    app.querySelector('[data-identity-action="confirm"]')?.addEventListener("click", () => {
      const name = readPlayerProfile().name;
      if (!name) {
        message = "Enter the name the Archive should use before confirming your identity.";
        render();
        return;
      }
      profile = writePlayerProfile({ ...profile, name });
      screen = "registration";
      message = "";
      render();
    });
    app.querySelector('[data-registration-action="back"]')?.addEventListener("click", () => {
      screen = "identity";
      render();
    });
    app.querySelector('[data-registration-action="enter"]')?.addEventListener("click", () => {
      profile = writePlayerProfile({ ...profile, codexIssued: true, fieldArrivalSeen: true });
      screen = "field";
      render();
    });
    app.querySelector('[data-field-action="talk"]')?.addEventListener("click", () => {
      fieldState.spoke = true;
      render();
    });
    app.querySelector('[data-field-action="complete"]')?.addEventListener("click", () => {
      fieldState.completeNotice = true;
      screen = "case";
      mountAtlanticCrossroadsPreview(app, {
        profile,
        onReturn: () => {
          screen = "field";
          render();
    ...
    app.querySelector('[data-field-action="return"]')?.addEventListener("click", () => {
      screen = "identity";
      render();
    });
    ...
    app.querySelectorAll("[data-identity-content]").forEach((input) =>
      input.addEventListener("input", (event) => {
        const path = event.currentTarget.dataset.identityContent;
        setAtPath(content, path, event.currentTarget.value);
        const warning = wordCountHint(
          event.currentTarget.value,
          path.includes("Dialogue") ? 280 : path.includes("subtitle") ? 130 : 120
        );
        const warningNode = app.querySelector(`[data-identity-warning="${CSS.escape(path)}"]`);
        if (warningNode) {
          warningNode.textContent = warning;
          warningNode.classList.toggle("is-visible", Boolean(warning));
        }
        scheduleContentSave();
      })
    );
    app
      ...
      ?.addEventListener("click", () => saveContent("Saved locally."));
    app.querySelector('[data-author-action="export"]')?.addEventListener("click", () => {
      saveContent("Draft saved locally and exported.");
      downloadContent("chronicle-identity-draft.json", content);
    });
    app.querySelector("#ciIdentityImport")?.addEventListener("change", async (event) => {
      const [file] = event.target.files;
      if (!file) return;
      try {
        content = await readImportedContent(file, CHRONICLE_IDENTITY_DEFAULTS);
        saveContent("Imported draft saved locally.");
        render();
      } catch {
        setAuthorStatus("Import failed. Choose a Chronicle identity JSON export.");
      }
    });
    app.querySelector('[data-author-action="reset"]')?.addEventListener("click", () => {
      if (!window.confirm("Discard this browser’s identity draft and restore repository defaults?"))
        return;
      clearLocalContent(CONTENT_KEY);
      content = clone(CHRONICLE_IDENTITY_DEFAULTS);
      render();
    });
  };
