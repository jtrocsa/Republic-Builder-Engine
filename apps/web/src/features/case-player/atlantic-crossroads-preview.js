...
const assets = {
  ...
  playerA: {
    ...
    up: new URL("../../assets/chronicle-sprites/field/chronicler-a-up-idle.png", import.meta.url)
      .href,
    side: new URL(
  ...
  playerB: {
    ...
    up: new URL("../../assets/chronicle-sprites/field/chronicler-b-up-idle.png", import.meta.url)
      .href,
    side: new URL(
...
const PROPS = [
  ...
  { kind: "palm", x: 9, y: 8, scale: 1.1, block: true },
  { kind: "tent", x: 7, y: 4, scale: 1.2, block: true },
  { kind: "crate", x: 8, y: 6, scale: 0.84, block: true },
  { kind: "lantern", x: 6, y: 6, scale: 0.75, block: false },
  { kind: "beacon", x: 5, y: 7, scale: 0.95, block: false },
...
function pos({ x, y }) {
  return `left:${(((x + 0.5) / COLS) * 100).toFixed(3)}%;top:${(((y + 0.5) / ROWS) * 100).toFixed(3)}%;`;
}
...
function terrain(tile) {
  return tile === "w" ? "water" : tile === "s" ? "sand" : tile === "p" ? "path" : "grass";
}
function tiles() {
  return MAP.flatMap((row, y) =>
    row
      ...
      .map(
        (tile, x) =>
          `<span class="ac-tile ac-tile--${terrain(tile)}" style="grid-column:${x + 1};grid-row:${y + 1}" aria-hidden="true"></span>`
      )
  ).join("");
}
function props() {
  return PROPS.map(
    (prop) =>
      `<img class="ac-prop ac-prop--${prop.kind}" style="${pos(prop)}--prop-scale:${prop.scale};" src="${assets[prop.kind]}" alt="" aria-hidden="true" />`
  ).join("");
}
function signals(found) {
  return SIGNALS.map(
    (signal) =>
      `<button class="ac-signal ${found.has(signal.evidenceId) ? "is-collected" : ""}" data-evidence-id="${signal.evidenceId}" style="${pos(signal)}" type="button" aria-label="Investigate ${signal.evidenceId}"><span>${signal.icon}</span></button>`
  ).join("");
}
function evidenceById(id) {
  return CASE.evidence.find((item) => item.id === id);
}
...
function fieldMarkup(profile, state) {
  const nearby = SIGNALS.find(
    (signal) => !state.found.has(signal.evidenceId) && near(state.position, signal)
  );
...
export function mountAtlanticCrossroadsPreview(app, { profile, onReturn }) {
  ...
  const state = {
    ...
    activeEvidence: null,
    mentorNote:
      "Your first task is not to explain the Atlantic world all at once. Establish context. Learn what the evidence can—and cannot—show.",
    answer: null,
  };
  const render = () => {
    app.innerHTML =
      screen === "briefing"
        ? caseBriefing(profile)
        : screen === "field"
          ? fieldMarkup(profile, state)
          : screen === "satchel"
            ? satchelMarkup(state)
            : screen === "check"
              ? sourceCheckMarkup(state)
              : recapMarkup(state);
    bind();
  };
  const move = (dx, dy) => {
    ...
    if (player) {
      player.style.cssText = pos(state.position);
      player.dataset.facing = state.facing;
      player.classList.add("is-moving");
      player.querySelector("img").src = playerSprite(profile, state.facing);
    }
    window.setTimeout(() => {
      state.moving = false;
      app.querySelector("#acPlayer")?.classList.remove("is-moving");
      if (state.queued) {
  ...
  const keys = (event) => {
    ...
    if ((event.key === "e" || event.key === "E") && screen === "field") {
      const signal = SIGNALS.find(
        (item) => !state.found.has(item.evidenceId) && near(state.position, item)
      );
      if (signal) {
        state.activeEvidence = signal.evidenceId;
        render();
      }
      return;
    }
  ...
  const bind = () => {
    app.querySelectorAll("[data-ac-action]").forEach((button) =>
      button.addEventListener("click", () => {
        ...
        if (action === "return") leave();
          ...
          screen = "briefing";
          render();
        } else if (action === "enter-case") {
          screen = "field";
          render();
        } else if (action === "open-satchel") {
          screen = "satchel";
          render();
        } else if (action === "back-to-case") {
          screen = "field";
          render();
        } else if (action === "inspect") {
          state.activeEvidence = button.dataset.evidenceId;
          render();
        } else if (action === "close-evidence") {
          state.activeEvidence = null;
          render();
        } else if (action === "secure-evidence") {
          state.found.add(button.dataset.evidenceId);
          state.activeEvidence = null;
          state.mentorNote =
            state.found.size === CASE.evidence.length
              ? "You have enough evidence for a first source check. Open your Codex and test the record."
              : "Good. Do not rush to a conclusion. Each record reveals something different.";
          render();
        } else if (action === "source-check") {
          screen = "check";
          state.answer = null;
          render();
          ...
          render();
        } else if (action === "finish-preview") {
          screen = "recap";
          render();
        }
      })
    );
    app.querySelectorAll(".ac-signal").forEach((button) =>
      button.addEventListener("click", () => {
        const id = button.dataset.evidenceId;
        const signal = SIGNALS.find((item) => item.evidenceId === id);
        if (signal && near(state.position, signal) && !state.found.has(id)) {
          state.activeEvidence = id;
          render();
        }
      })
    );
