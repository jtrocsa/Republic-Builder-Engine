...
The files under `apps/web/src/features/*` (`chronicle-institute.js`, `chronicle-identity.js`, `atlantic-crossroads-preview.js`) and `apps/web/src/engine/content/author-content-store.js` / `apps/web/src/engine/player/player-profile-store.js` are **not imported by `main.js`** — they are a complete, orphaned second implementation of the onboarding→field→case-player loop (six files total), containing two more dead Author Mode implementations on top of `main.js`'s own broken one. Before assuming feature logic lives in one of these files, check whether `main.js` actually imports it. When extending gameplay, the pragmatic move is almost always to edit `main.js` directly rather than wire in an orphaned module, unless the user is deliberately doing the modularization work. Per `docs/architecture/ARCHITECTURE-REVIEW-AND-SIMPLIFICATION.md`, this dead code is confirmed zero-risk to delete but has not been deleted yet — don't assume it's gone just because it's documented as dead.
...
| Feature-level UI (currently orphaned, see above)   | `apps/web/src/features/`                                                                  |
...
| Images, maps, audio, icons **(live)**              | `apps/web/src/assets/` — 62 real files, referenced via `new URL(..., import.meta.url)`    |
| Images, maps, audio, icons (placeholder scaffold)  | `assets/` (repo-root) — currently 100% `.gitkeep` files, no real assets live here          |
