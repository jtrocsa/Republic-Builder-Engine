...
- Buildings: Medieval Fantasy Town `1.png` (print shop, family residence, statehouse
  ...
  chapel, frontier dispatch post watchtower) — all pulled from that pack's *unlabeled*
...
| Medieval Fantasy Town    | 16    | `KEEP-CONDITIONAL`                    | Riverbend ground/buildings; also supplies Common Cause's (case-007) unlabeled building silhouettes as of decision log `0032` — its *signed* buildings ("Adventurer's Guild", "The Sword & Shield") remain off-limits for Philadelphia, only unlabeled cottage/hall/church/watchtower tiles are used |
