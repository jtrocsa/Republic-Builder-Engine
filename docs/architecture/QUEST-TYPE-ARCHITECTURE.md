...
## 5. Phase 10 (2026-07-12): MCQ wired live — three types still unreached

`main.js` now imports `quest-types/`: MCQ is a real, player-reachable screen. Sequencing, Evidence Organizing, and HIPP remain built-and-tested-but-unreached, the same structural shape the now-deleted `apps/web/src/features/` island had before removal (`docs/migrations/DEAD-CODE-REMOVAL.md`) — this is still true for 3 of 4 types.

**What was wired:** a new `practice-check` screen (`VALID_SCREENS`, the render switch, `practiceCheckScreen()` in `main.js`), reached via a "Practice Check →" button added to the Case 1.01 field's existing Evidence Channel aside panel (a plain UI button, not a physical field object — no proximity/collision/movement code was touched, deliberately avoiding `main.js`'s most regression-prone surface). Renders `UNIT_01_MCQ_QUESTS` through the real `renderQuest("mcq", ...)`/`gradeQuest("mcq", ...)` entry points. Answers persist via a new `questResponses: {}` field on `DEFAULT_PROGRESS` (`chronicle-progress-store.js`), read/graded on every render, saved through a new `[data-mcq-quest]` branch in `main.js`'s existing `change` listener (radio inputs from `renderMcqQuest()` don't carry `data-action`, unlike most of `main.js`'s click-dispatched UI). New CSS in `global.css` scopes `.quest-choices .choice` separately from the shared `.choice` class used by `reviewScreen()`'s letter-badge MCQ layout, since `renderMcqQuest()`'s markup shape doesn't match that rule's 3-column grid.

**Verified live** (not just the preview harness) via a scripted Playwright pass against the real dev server: navigation from the field, all 3 real MCQ items rendering, correct/incorrect grading against real `explanation` text, the answered counter, **answer persistence across a full page reload**, and the back link. `npm run test` 179/179 (unchanged — no new exported pure logic was introduced; `practiceCheckScreen()` isn't unit-tested, matching the existing convention that `main.js` screen-assembly functions like `villageActivityScreen()` aren't either), `npm run validate:content` 21/21, `npm run lint` unchanged baseline, `npm run build` succeeds with a larger bundle (275 KB vs 243 KB, confirming real inclusion).

**Still not done at the time:** Sequencing, Evidence Organizing, and HIPP were not yet reachable from the running game — closed the same day in Phase 11, below.

## 5a. Phase 11 (2026-07-12, same day): remaining three quest types wired live

Owner asked explicitly to see all four built quest types in-game, "even as test features." `practiceCheckScreen()` was expanded from one MCQ-only section into four sections — **Multiple choice, Sequencing, Evidence organizing, HIPP source analysis** — all inside the same `practice-check` screen from Phase 10 (one entry point, no new screens or buttons added). All four now render `UNIT_01_MCQ_QUESTS` / `UNIT_01_SEQUENCING_QUESTS` / `UNIT_01_EVIDENCE_ORGANIZING_QUESTS` / `UNIT_01_SOURCE_ANALYSIS_QUESTS` through the existing `renderQuest`/`gradeQuest` lookup — this phase added zero quest-type code, purely wiring.

**What was wired, by type:**

- **Sequencing** — `renderSequencingQuest()`'s draggable `[data-sequence-item]` `<li>` elements needed real drag-and-drop. Extended `main.js`'s existing native-HTML5-DnD listeners (`dragstart`/`dragover`/`dragleave`/`drop` — already handling `[data-map-piece]`, `[data-cargo-card]`, `[data-region-card]`, `[data-empire-card]`) with a `[data-sequence-item]` branch on each, following the same `.closest()` + `dataset` + `is-over` class pattern already used for the map-piece/triangle-leg/region-column drop zones. On drop, reads the live DOM order of sibling `[data-sequence-item]` elements, splices the dragged item to its new position, and writes the result to `questResponses[questId].order`.
- **Evidence Organizing** — same DnD extension pattern for `[data-evidence-source]` → `[data-evidence-slot]`. On drop, writes `questResponses[questId].placements[sourceId] = slotId`, clearing any other source previously occupying that slot first (mirroring the existing map-piece single-occupant logic). The optional reflection textarea got a `[data-evidence-reflection]` branch in the existing `change` listener.
- **HIPP** — needed only a `[data-hipp-option]` branch in the same `change` listener as the Phase 10 MCQ branch (same radio-input shape), writing `questResponses[questId].selected[promptId] = optionId`.

No changes to `DEFAULT_PROGRESS`/`readProgress()` in `chronicle-progress-store.js` — the flat `questResponses: {}` map added in Phase 10 already accommodates arbitrary per-quest-type state shapes without a store change. New CSS: `.quest-section-heading`, `.sequence-item`/`.quest-sequence-list`, `.evidence-card`/`.quest-evidence-sources`/`.evidence-slot`/`.quest-evidence-slots`/`.quest-reflection`, `.quest-document`/`.quest-hipp-prompts`/`.hipp-prompt`/`.hipp-option`, plus `.is-over` variants for the two new drop targets — following the existing `.leg-drop`/`.region-column` visual pattern rather than inventing a new drop-zone style.

**Verified live** via a scripted Playwright pass against the real dev server: all 4 sections render with real Case 1.01 content; MCQ grades live on selection; a native-event-dispatched drag from the first sequence item onto the last correctly reorders the list and grades live; a native-event-dispatched drag from the first evidence source onto the first slot fills it and grades live; a HIPP radio selection grades live (1/2 points on the seeded document); **all four quest types' state persists correctly across a full page reload** (confirmed by reading the raw `questResponses` blob back out of `localStorage` after reload); the back link returns to the field. `npm run test` 179/179 (unchanged — pure wiring, no new pure logic), `npm run validate:content` 21/21, `npm run lint` unchanged baseline (the one pre-existing `main.js` error confirmed pre-existing via `git stash` diff before/after), `npm run build` succeeds.

**Result: all four quest types are now live, player-reachable, and persist state**, off one `practice-check` screen explicitly framed as practice/test content that doesn't gate Preservation Case progress.

## 6. Recommended next step

Quest-type wiring is done — no quest type remains built-but-unreached. The next approved product step reverts to `POST-MINIMAL-ARCHITECTURE-REASSESSMENT.md` §7's original recommendation: build out the Atlantic or Hispaniola badge area, now using these quest types where they fit instead of hand-duplicating the pattern a fourth time. The mini-games layer (`cargo-sorting`, `storm-navigation`) also remains unwired into `main.js` and is a separate, smaller candidate for the same treatment whenever it fits a pacing beat.
