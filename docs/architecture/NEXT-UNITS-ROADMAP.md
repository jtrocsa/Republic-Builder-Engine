...
`apps/web/src/mini-games/` (`cargo-sorting.js`, `storm-navigation.js`) is built, tested, and completely unwired into `main.js` — explicitly non-rubric-scored pacing/reward content, not a quest type. `QUEST-TYPE-ARCHITECTURE.md` §6 already flags it as "a separate, smaller candidate for the same treatment whenever it fits a pacing beat." Worth wiring in opportunistically alongside whichever future unit's content work has a natural pacing gap for it — not a reason to open a dedicated phase on its own.
