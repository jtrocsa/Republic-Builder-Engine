...
- Two distinct reset paths exist with very different blast radius: `resetCaseOneDemo()` surgically resets only `case-001`-related fields while preserving `profile`; the blunt `"reset"` action wipes the entire save via `resetProgress()`.
  - **Correction, 2026-08-22 (Phase 90D).** The first half of that sentence was wrong from the day it was written, and this file's own method is verification against source. `resetCaseOneDemo()` opened with `progress = resetProgress()` — the whole save, every unit — and then re-seeded a handful of Case 1.01 fields, which is what made it read as surgical on a quick pass. Since Phase 90D it is surgical, via the exported `resetCaseState()`, and `resetProgress()` has 2 call sites here rather than 3. Left in place rather than rewritten, per the `0006` convention: an audit records what was believed, and the propagation is the finding. See Spine Review Part 6A and decision log `0082`. Line numbers throughout this bullet are from the 2026-07 tree and are long stale.
