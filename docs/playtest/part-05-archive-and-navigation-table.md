...
| 6   | S2  | `unclear`      | Six of the seven unit tabs — this screen's only route to any unit but the selected one — sat **below the fold**, and the page grew a 1001px scroll height against a 720px viewport to hold them. Measured, not estimated; see below.                                                                | Fixed, then wrong again. **Re-opened and closed in Phase 117** (`0116`) — see below. |
...
## Finding 6, again — and what the measurement above could not see

**Phase 117, decision log [`0116`](../decision-log/0116-the-controls-sat-under-the-prose.md).** The
table above is a true measurement of the wrong state, and so is the one Phase 107 took when it
re-checked this at eight units. Both read **Unit 1 selected with two cases unlocked**, which is the
default save and the one state on this screen that cannot show the defect.

The strip rendered **under the selected unit's guiding question**, so where this screen's only
navigation landed was a function of how long an author had written. And the route panel did the same
thing with the button that starts the mission: it sat under the case summary, which runs **18 words
in Unit 1 and 79 in Unit 8** — five lines against sixteen in the route panel.

Re-measured with the whole course unlocked, every unit selected in turn. `✗` is below the fold.
...
**1366×768:** Unit 7 put two tabs, the legend and the Chronotravel button below the fold; Unit 8 put
the legend and the button below it, clipped to its top 22–29 pixels. Everything through Unit 6 was
clear.

So the finding above understated itself. It is not that six of seven tabs were below the fold on one
screen size in 2026 — it is that **this screen put every control it has underneath a block of prose
whose length nobody controls**, and the two later checks both happened to select the shortest one.

Fixed by moving the navigation above the prose in both columns, and by pinning the left column to the
top of its grid row instead of centring it against a panel whose height is that summary — which had
been sliding the strip **89px between Unit 1 and Unit 8, out from under the cursor that had just
clicked it**. The strip's top edge now takes one value across all eight periods.

The guard in `archive-navigation.spec.js` keeps the original case, with a note saying which state it
reads, and walks all eight periods at both sizes beside it.
