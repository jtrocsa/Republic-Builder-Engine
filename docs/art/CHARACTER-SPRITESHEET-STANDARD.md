...
`walkCycleProfile({ frameSize })` takes an explicit `{ width, height }` rather than hard-coding `48`. The default (`DEFAULT_FRAME_SIZE`) is 48×48 as a proposed house standard — **not**, as this line previously claimed, "to match every current asset"; see the frame-size note above for what the assets actually are. Nothing in the module assumes that value — a future character sheet authored at a different resolution (for instance, a larger boss/mentor sprite) can pass its own `frameSize` without a code change.
