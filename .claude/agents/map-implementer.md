...
- Don't adopt Phaser or Tiled — both remain deferred (see `docs/architecture/ARCHITECTURE-QUICKREF.md` §8). If a new map's complexity genuinely can't be handled by a hand-coded array, stop and flag that as a possible forcing function rather than adopting either tool yourself.
