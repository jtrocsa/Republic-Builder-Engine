...
- Sprite convention for new characters (Phase 60): four horizontal walk strips plus a still portrait, `<character>-{down,up,left,right}.png` and `<character>-portrait.png`, every column on the shared 48×56 canvas with feet on row 49. Column 0 is a standing pose for that direction; columns 1..N are the walk cycle. Don't hand-author these — add the character to `scripts/assets/character-manifest.js` and run `npm run assets:build-characters`, which normalizes body height and foot line across the whole cast. The old `<character>-<facing>[-step].png` / `{down, up, side}` convention is retired; only Unit 3's frozen placeholder sources still use it, as build input.
