...
// poolSourcesForCopy()) into the {label, attribution, excerpt, fullText}
// fields both the evidence-organizing and HIPP forms autofill from — HIPP's
// document-text field prefers fullText over excerpt when one has been
// transcribed (see the [data-copy-hipp-source] branch of handleAppChange()).
// Uses real primary-source-library entries rather than fabricated fixtures,
// matching this repo's convention of testing against real cited content.
describe("resolvePoolSourceFields", () => {
  it("maps a text source's creator/date/excerpt/fullText", () => {
    const result = resolvePoolSourceFields("text:u01-columbus-first-voyage-letter");
    ...
    // This source has real transcribed full text — fullText must be the
    // actual verbatim letter, not a repeat of the short excerpt above.
    expect(result.fullText).toBeTruthy();
    expect(result.fullText).not.toEqual(result.excerpt);
    expect(result.fullText).toContain("San Salvador");
  });

  it("maps a visual source's citation/description instead of creator/date/excerpt, with no fullText", () => {
